...
class FurnituredApartmentCubit extends Cubit<FurnituredApartmentState> {
  FurnituredApartmentCubit(this.homePropertiesRepo) : super(FurnituredApartmentInitial());
  final HomePropertiesRepo homePropertiesRepo;
  Future<void> fetchFurnituredProperties() async {
    emit(FurnituredApartmentLoading());
    var result = await homePropertiesRepo.fetchHomeProperties(fields:'?category=furnished apartment');
    result.fold((failure) {
